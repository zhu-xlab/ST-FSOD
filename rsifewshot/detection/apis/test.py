...
from rsidet.apis.test import collect_results_cpu, collect_results_gpu
from rsidet.utils import get_root_logger
from torch.utils.data import DataLoader
from ..wandb.det_wandb_visualizer import DetWandbVisualizer
...
def single_gpu_test(model: nn.Module,
    ...
    The model forward function requires `mode`, while in rsidet it requires
    ...
    if is_module_wrapper(model):
        wandb_cfg = model.module.wandb_cfg if hasattr(model.module, 'wandb_cfg') else None
    else:
        wandb_cfg = model.wandb_cfg if hasattr(model, 'wandb_cfg') else None

    visualizer = DetWandbVisualizer(dataset, wandb_cfg) if wandb_cfg is not None else None
    ...
    for i, data in enumerate(data_loader):
        ...
        if visualizer is not None and i % visualizer.interval == 0:
            if type(result[0]) == list:
                visualizer.process_vis_data({
                    'vis|dets_test': {'imgs': imgs, 'bbox_results': result}
                })
            elif type(result[0]) == dict:
                visualizer.process_vis_data({
                    'vis|dets_test': {'imgs': imgs, 'bbox_results': [temp['bbox_results'] for temp in result]},
                    'vis|dets_test_proposals': {'imgs': imgs, 'bbox_results': [temp['proposal_results'] for temp in result]}
                })
