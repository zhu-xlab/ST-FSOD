...
from rsidet.datasets.builder import DATASETS, worker_init_fn
from rsidet.datasets.dataset_wrappers import (ClassBalancedDataset,
                                             ConcatDataset, RepeatDataset)
from rsidet.datasets.samplers import (DistributedGroupSampler,
                                     DistributedSampler, GroupSampler)
from torch.utils.data import DataLoader, Dataset, Sampler

from rsifewshot.utils.infinite_sampler import (DistributedInfiniteGroupSampler,
...
def build_dataset(cfg: ConfigDict,
    ...
    if rank == 0 and save_dataset:
        ...
        save_dataset_path = cfg.pop('save_dataset_path', osp.join(work_dir, f'{timestamp}_saved_data.json'))
        save_dataset_dir = '/'.join(save_dataset_path.split('/')[:-1])
