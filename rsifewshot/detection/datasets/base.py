...
from rsidet.datasets.custom import CustomDataset
from rsidet.datasets.pipelines import Compose
from terminaltables import AsciiTable

from rsifewshot.utils import get_root_logger
...
class BaseFewShotDataset(CustomDataset):
    ...
    def __init__(self,
                 ...
                 balance_base_novel: bool = False,
                 add_unlabeled_data: bool = False,
                 **kwargs) -> None:
        ...
        self.balance_base_novel = balance_base_novel
        ...
        if not test_mode:
            ...
            if self.balance_base_novel:
                self.data_infos = self._balance_base_novel(self.data_infos)

            if add_unlabeled_data and hasattr(self, 'ignored_data_infos'):
                self.data_infos += self.ignored_data_infos
    ...
    def _filter_annotations(self, data_infos: List[Dict],
        ...
        for idx, data_info in enumerate(data_infos):
            ...
            if len(selected_instance_indices) == 0:
                cur_data_info = copy.deepcopy(data_info)
                cur_data_info['ann'] = {'bboxes': data_info['ann']['bboxes'][[], :],
                                        'labels': data_info['ann']['labels'][[]]}
                # cur_data_info['ann'] = {}
                # cur_data_info['ann']['bboxes'] = cur_data_info['ann']['bboxes'][]
                self.ignored_data_infos.append(cur_data_info)
                continue
    ...
    def _balance_base_novel(self, data_infos):

        novel_class_names = self.SPLIT[f'NOVEL_CLASSES_SPLIT{self.split_id}']
        num_novel_imgs = 0
        novel_idxs = []
        for idx, data_info in enumerate(data_infos):
            ann = data_info['ann']
            ...
            for label in ann['labels']:
                if self.CLASSES[label] in novel_class_names:
                    has_novel = True

            num_novel_imgs += has_novel
            if has_novel:
                novel_idxs.append(idx)

        num_base_imgs = (len(data_infos) - num_novel_imgs)
        novel_repeat_times = round(num_base_imgs / num_novel_imgs)
        data_infos += [data_infos[idx] for idx in novel_idxs] * (novel_repeat_times - 1)
    ...
    def __repr__(self) -> str:
        ...
        for idx in range(len(self)):
            label = self.get_ann_info(idx).get('labels', None)
