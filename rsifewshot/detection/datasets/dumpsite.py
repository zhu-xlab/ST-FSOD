...
from Dataset4EO.datasets import load as load_datapipe
...
class FewShotDumpsiteDataset(BaseFewShotDataset):
    """Dumpsite dataset for few shot detection.

    Args:
        classes (str | Sequence[str]): Classes for model training and
            provide fixed label for each class. When classes is string,
            it will load pre-defined classes in `FewShotDumpsiteDataset`.
    ...
    def ann_cfg_parser(self, ann_cfg: List[Dict]) -> List[Dict]:
        ...
        Overwrite this function from the base class.
        Support type of ``datapipe'' in ann_cfg, by which we use Dataset4EO to load the data_infos
        """
        ...
        for ann_cfg_ in ann_cfg:
            ...
            assert ann_cfg_['type'] in ['ann_file', 'saved_dataset', 'datapipe'], \
    ...
    def load_annotations(self, ann_cfg: List[Dict]) -> List[Dict]:
        ...
        for ann_cfg_ in ann_cfg:
            ...
            elif ann_cfg_['type'] == 'datapipe':
                datapipe_name = ann_cfg_['datapipe_name']
                datapipe_root = ann_cfg_['datapipe_root']
                datapipe_split = ann_cfg_['datapipe_split']

                datapipe = load_datapipe(datapipe_name, root=datapipe_root, split=datapipe_split)
                # self.CLASSES = datapipe.get_classes()
                data_infos += self.load_annotations_xml_by_datapipe(datapipe, self.CLASSES)
            else:
                raise ValueError(
    ...
    def load_annotations_xml_by_datapipe(self, datapipe, classes):
        ...
        for i, item in enumerate(datapipe):
            ...
            ann_info = self._get_xml_ann_info(xml_path, classes)
            data_infos.append(
                dict(id=img_id,
                     filename=filename,
                     width=width,
...
@DATASETS.register_module()
class FewShotDumpsiteCopyDataset(FewShotDumpsiteDataset):
    """Copy other Dumpsite few shot datasets' `data_infos` directly.
    ...
    Args:
        ann_cfg (list[dict] | dict): contain `data_infos` from other
            dataset. Example: [dict(data_infos=FewShotDumpsiteDataset.data_infos)]
    """
    ...
    def ann_cfg_parser(self, ann_cfg: Union[List[Dict], Dict]) -> List[Dict]:
        ...
        Args:
            ann_cfg (list[dict] | dict): contain `data_infos` from other
                dataset. Example:
                [dict(data_infos=FewShotDumpsiteDataset.data_infos)]
        ...
        if isinstance(ann_cfg, dict):
            assert ann_cfg.get('data_infos', None) is not None, \
                ...
                f'FewShotDumpsiteCopyDataset require data_infos.'
...
@DATASETS.register_module()
class FewShotDumpsiteDefaultDataset(FewShotDumpsiteDataset):
    """Dataset with some pre-defined Dumpsite annotation paths.

    :obj:`FewShotDumpsiteDefaultDataset` provides pre-defined annotation files
    ...
    different from :obj:`FewShotDumpsiteDataset`. The `ann_cfg' should contain
    ...
    voc_benchmark = {
        f'SPLIT{split}_{shot}SHOT': [
            ...
            for class_name in DUMPSITE_SPLIT[f'ALL_CLASSES_SPLIT{split}']
        ]
    ...
    def ann_cfg_parser(self, ann_cfg: List[Dict]) -> List[Dict]:
        ...
        return super(FewShotDumpsiteDataset, self).ann_cfg_parser(new_ann_cfg)
