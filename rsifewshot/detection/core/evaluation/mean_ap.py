...
import numpy as np
from rsidet.core import average_precision, print_map_summary
from rsidet.core.evaluation.mean_ap import (get_cls_results, tpfp_default,
                                           tpfp_imagenet)
...
def eval_map(det_results: List[List[np.ndarray]],
    ...
    :func:`eval_map` in rsidet predefines the names of classes and thus not
    ...
    Args:
        ...
        use_legacy_coordinate (bool): Whether to use coordinate system in
            rsidet v1.x. which means width, height should be
    ...
    for i in range(num_classes):
        ...
        if num_dets == 0: # No detection on the current class
            eval_results.append({
                'num_gts': num_gts,
                'num_dets': num_dets,
                'recall': recalls,
        ...
        thre_idx = (precisions * recalls).argmax()
        thre = cls_dets[sort_inds][thre_idx, -1]
