...
from rsidet.core import DistEvalHook, EvalHook
from rsidet.core.mask.structures import polygon_to_bitmap


@HOOKS.register_module()
class RSIDetWandbHook(WandbLoggerHook):
    ...
    def __init__(self,
                 ...
                 BGR2RGB=False,
                 without_mask=True,
                 wandb_boxes_resize_factor=1.,
                 **kwargs):
        super(RSIDetWandbHook, self).__init__(init_kwargs, interval, **kwargs)
        ...
        self.wandb_boxes_resize_factor = wandb_boxes_resize_factor
    ...
    def before_run(self, runner):
        super(RSIDetWandbHook, self).before_run(runner)
        ...
        if self.log_evaluation or self.log_checkpoint_metadata:
            if self.eval_hook is None:
                ...
                runner.logger.warning(
                    ...
                    'MMDetWandbHook, `EvalHook` or `DistEvalHook` in rsidet '
    ...
    def after_train_epoch(self, runner):
        super(RSIDetWandbHook, self).after_train_epoch(runner)
    ...
    def _result2wandb_image(self, img, bbox_result, segm_result):
        # Get labels
    ...
    def _vis_dets(self, prefix, img, bbox_results, segm_results=None):
    ...
    def _add_ground_truth(self, runner):
        ...
        from rsidet.datasets.pipelines import LoadImageFromFile
        img_loader = None
    ...
    def _get_wandb_bboxes(self, bboxes, labels, log_gt=True, class_id_to_label=None):
        ...
        for bbox, label in zip(bboxes, labels):
            ...
            resize_factor = 1
            if self.wandb_boxes_resize_factor is not None:
                resize_factor = self.wandb_boxes_resize_factor
            position = dict(
                minX=int(bbox[0] * resize_factor),
                minY=int(bbox[1] * resize_factor),
                maxX=int(bbox[2] * resize_factor),
                maxY=int(bbox[3] * resize_factor))
